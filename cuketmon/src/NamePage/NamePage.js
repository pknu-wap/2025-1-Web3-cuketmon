...
function NamePage() {
  ...
  const handleBlur = () => {
    if (name.trim().length > 0) {
      alert("커켓몬이 생성되었습니다.")
      navigate("/mypage"); //마이페이지로 이동해 커켓몬이 제대로 생성되었는지 확인할 수 있게!
    }
  };
  ...
  return (
    <div className="namePage">
      <div className="nameInputBox">
        <p>Your name?</p>
        {pokemonImage && <img src={pokemonImage} alt="커켓몬 이미지" className="cukemonImage" />}
        ...
        <div className="nameInput">
          ▶<input 
            ...
            onBlur={handleBlur}  
          />
          <p id='ramainWord'>{name.length}/{maxLength}자</p>
        </div>
